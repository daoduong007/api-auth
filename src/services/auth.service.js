...
const registerUser = async (userbody) => {
  ...
  if (user) {
    const permission = await userService.savePermission(
      user._id,
      userbody.permission_type,
      userbody.description,
    );
  }
  return user;
};
...
const saveRefreshToken = async (userid, token) => {
  const refreshToken = await refresh_token.findOneAndUpdate(
    { user_id: userid },
    {
      user_id: userid,
      token: token,
...
const deleteRefreshToken = async (userid) => {
  const refreshToken = await refresh_token.findOneAndRemove({
    user_id: userid,
  });
  return refreshToken;
};
